...
bool RadarDetection::get_vaild_preproc()
{
...
bool RadarDetection::get_valid_postproc()
{
    valid = valid && (!is_static);
    return valid;
}

bool RadarDetection::get_static(double ego_speed_)
{
  if (rel_speed != 0)
  {
    get_speed();
...
double RadarDetection::get_tangential_vel()
{
  trangential_vel = (lateral_vel - range_vel * sin_angle) / (cos_angle + epsilon);
}

std::vector<double> RadarDetection::get_cartesian_pos()
{
...
std::vector<double> RadarDetection::get_cartesian_vel()
{
...
double RadarDetection::get_speed()
{
...
std::string RadarDetection::toString() {
